...
public class AccountsServiceTest {
  ...
  public void makeTransfer_should_fail_when_accountDoesNotExist() {
      ...
      try {
          ...
          assertThat(ife.getMessage()).isEqualTo("Not enough funds on account " + accountFromId + " balance=0");
      } catch (TransferBetweenSameAccountException tbsae) {
          assertThat(tbsae.getMessage()).isEqualTo("From and To accounts can't be same");
      }
  ...
  public void makeTransfer_should_fail_when_accountNotEnoughFunds() {
      ...
      try {
          ...
          assertThat(ife.getMessage()).isEqualTo("Insufficient funds on account " + accountFromId + " balance=0");
      } catch (AccountNotFoundException anfe) {
          assertThat(anfe.getMessage()).isEqualTo("Account " + accountToId + " not found.");
      } catch (TransferBetweenSameAccountException tbsae) {
          assertThat(tbsae.getMessage()).isEqualTo("From and To accounts can't be same");
