...
public class AccountsControllerTest {
  ...
  private void verifyAccountBalance(final String accountId, final BigDecimal balance) throws Exception {
	  ...
	  public void makeTransferAccountFromNotFound() throws Exception {
	    ...
	    makeTransferWithContent("{\"fromAccount\":\"Id-1\",\"toAccount\":\"Id-2\",\"amount\":1000}")
	  ...
	  public void makeTransferAccountToNotFound() throws Exception {
	    this.mockMvc.perform(post("/v1/accounts").contentType(MediaType.APPLICATION_JSON)
	    	      .content("{\"accountId\":\"Id-1\",\"balance\":1000.20}")).andExpect(status().isCreated());
	    makeTransferWithContent("{\"fromAccount\":\"Id-1\",\"toAccount\":\"Id-2\",\"amount\":1000}")
	            .andExpect(status().isNotFound());
	  }
	  ...
	  public void makeTransferSameAccount() throws Exception {
	    ...
	    makeTransferWithContent("{\"fromAccount\":\"Id-1\",\"toAccount\":\"Id-1\",\"amount\":1000}")
	            .andExpect(status().isBadRequest());
	  }
	  ...
	  public void makeTransferNegativeAmount() throws Exception {
		  this.mockMvc.perform(post("/v1/accounts").contentType(MediaType.APPLICATION_JSON)
			      .content("{\"accountId\":\"Id-1\",\"balance\":100.50}")).andExpect(status().isCreated());
		  this.mockMvc.perform(post("/v1/accounts").contentType(MediaType.APPLICATION_JSON)
			      .content("{\"accountId\":\"Id-2\",\"balance\":1000.50}")).andExpect(status().isCreated());

	    makeTransferWithContent("{\"fromAccount\":\"Id-1\",\"toAccount\":\"Id-2\",\"amount\":-50}")
	            .andExpect(status().isBadRequest());
	  }

		
		  @Test public void makeTransferEmptyBody() throws Exception {
		  makeTransferWithContent("{}") .andExpect(status().isBadRequest()); }
	  ...
	  public void makeTransferZeroBalanceAfterTransfer() throws Exception {
		  this.mockMvc.perform(post("/v1/accounts").contentType(MediaType.APPLICATION_JSON)
			      .content("{\"accountId\":\"Id-1\",\"balance\":2000.20}")).andExpect(status().isCreated());
		  this.mockMvc.perform(post("/v1/accounts").contentType(MediaType.APPLICATION_JSON)
			      .content("{\"accountId\":\"Id-2\",\"balance\":100}")).andExpect(status().isCreated());

	    makeTransferWithContent("{\"fromAccount\":\"Id-1\",\"toAccount\":\"Id-2\",\"amount\":2000.20}")
	            .andExpect(status().isOk());
	  ...
	  public void makeTransferBetweenAccountsPositiveBalanceAfterTransfer() throws Exception {
		  ...
		  this.mockMvc.perform(post("/v1/accounts").contentType(MediaType.APPLICATION_JSON)
			      .content("{\"accountId\":\"Id-1\",\"balance\":5100.21}")).andExpect(status().isCreated());
		  this.mockMvc.perform(post("/v1/accounts").contentType(MediaType.APPLICATION_JSON)
			      .content("{\"accountId\":\"Id-2\",\"balance\":6000}")).andExpect(status().isCreated());

	    makeTransferWithContent("{\"fromAccount\":\"Id-1\",\"toAccount\":\"Id-2\",\"amount\":5000}")
	            .andExpect(status().isOk());
